...
$(function () { 
    ...
    const countArray = Object.keys(count);
    const countNumber = Object.keys(count).length;
    
    // create a function that returns the object with the highest occurences(count)
    const fundayDestination = _.max(Object.keys(count), function(highest) {
        console.log(count[highest]);
        return (count[highest]);
    });
    
    // create a function that returns a random object when all choices have an equal count
    const randomNumber = function (number) {
        return Math.floor(Math.random() * number.length);
    }

    const getRandomDestination = function (number) {
        const i = randomNumber(number);
        console.log(number)
    ...
    // render the object on the page 
        if (count > 1) {
           return $('.results').html(`<h1 class="choice">Check out ${fundayDestination}</h1>`);
            ...
            return $('.results').html(`<h1 class="choice">Check out ${getRandomDestination(countArray)}</h1>`);
        }
    });
